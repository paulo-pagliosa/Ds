...
public:
  ...
  HOST DEVICE explicit Color(float r, float g, float b, float a = 1) {
    setRGB(r, g, b, a);
  }

  /// Constructs a Color object from c[4].
  HOST DEVICE explicit Color(const float *c) { setRGB(c); }

  /// Constructs a Color object from (r, g, b, a).
  HOST DEVICE explicit Color(int r, int g, int b, int a = 255) {
    setRGB(r, g, b, a);
  }

  /// Constructs a Color object from v.
  template <typename V> HOST DEVICE explicit Color(const V &v) { setRGB(v); }

  /// Sets this object to (r, g, b, a).
  HOST DEVICE void setRGB(float r, float g, float b, float a = 1) {
    this->r = r;
    this->g = g;
    this->b = b;
    this->a = a;
  ...
  HOST DEVICE void setRGB(const float *c) {
    r = c[0];
    g = c[1];
    b = c[2];
    a = c[3];
  ...
  HOST DEVICE void setRGB(int r, int g, int b, int a = 255) {
  ...
  template <typename V> HOST DEVICE void setRGB(const V &v) {
    r = float(v.x);
    g = float(v.y);
    b = float(v.z);
    a = float(v.w);
  ...
  template <typename V> HOST DEVICE auto &operator=(const V &v) {
    setRGB(v);
    return *this;
  }

  /// Returns this object + c.
  HOST DEVICE Color operator+(const Color &c) const {
  ...
  HOST DEVICE Color operator*(float s) const {
    return Color(r * s, g * s, b * s);
  }

  /// Returns the i-th component of this object.
  HOST DEVICE const auto &operator[](int i) const { return (&r)[i]; }

  /// Returns a reference to the i-th component of this object.
  HOST DEVICE auto &operator[](int i) { return (&r)[i]; }

  /// Returns a pointer to the elements of this object.
  HOST DEVICE explicit operator const float *() const { return &r; }

  /// Returns a reference to this object += c.
  HOST DEVICE auto &operator+=(const Color &c) {
    r += c.r;
    g += c.g;
    b += c.b;
    return *this;
  ...
  HOST DEVICE auto &operator*=(float s) {
    r *= s;
    g *= s;
    b *= s;
    return *this;
  ...
  HOST DEVICE bool equals(const Color &c,
                          float eps = math::Limits<float>::eps()) const {
    return math::isNull(r - c.r, g - c.g, b - c.b, eps);
  }

  HOST DEVICE bool operator==(const Color &c) const { return equals(c); }

  /// Returns true if this object is not equals to c.
  HOST DEVICE bool operator!=(const Color &c) const { return !operator==(c); }

  void print(const char *s) const { printf("%srgb(%g,%g,%g)\n", s, r, g, b); }
...
HOST DEVICE inline auto operator*(real s, const Color &c) {
  return c * float(s);
}
...
constexpr inline uint32_t packColor(uint32_t r, uint32_t g, uint32_t b,
                                    uint32_t a = 255) {
...
inline Color unpackColor(uint32_t c) {
